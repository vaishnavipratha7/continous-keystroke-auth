...
# CONFIGURATION: Window size chosen based on EER comparison
# Testing showed window_size=50 performs better than window_size=30 (EER: 24.43% vs ~26%)
WINDOW_SIZE = 50
...
def run_validation():
    ...
    print(f"Using window_size={WINDOW_SIZE} digraphs (optimized for EER)")
    ...
    # Compute FAR, FRR at different thresholds to find EER
    ...
    with open(REPORT_PATH, 'w') as f:
        ...
        f.write(f"Window size: {WINDOW_SIZE} digraphs (optimized for EER)\n\n")
        ...
        f.write(f"Equal Error Rate (EER): {eer * 100:.2f}% at threshold = {eer_threshold:.4f}\n")
